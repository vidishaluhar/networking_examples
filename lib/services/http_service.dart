...
class HttpService {
  ...
  static Future<List<HttpModel>> fetchDataFromHttpApi() async
  {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return httpData.map((e) => HttpModel.fromJson(e),).toList();
      }
      else {
  ...
  static Future<HttpModel> sendData() async {
    final http.Response response=await http.post(Uri.parse("https://jsonplaceholder.typicode.com/albums/"),
        headers: <String, String>{
          'Content-type': 'application/json; charset=UTF-8'
        }, body: jsonEncode(HttpModel(title: 'Vidisha', userId: 11,)..toJson()));
  ...
  static Future<HttpModel> updateMapData() async {
    final http.Response response=await http.put(Uri.parse("https://jsonplaceholder.typicode.com/albums/"),
        headers: <String, String>{
          'Content-type': 'application/json; charset=UTF-8'
        }, body: jsonEncode(HttpModel(title: 'Priyanshi', userId: 111,)..toJson()));
    print(response.statusCode);
    print(response.body);
    if(response.statusCode==200)
    {
